...
complements the symbolic pipeline and independently rediscovers the hand-crafted
style structure. All code and data references are in the accompanying ZIP file.
\end{abstract}
...
$d = 64$, 150 epochs, 80/10/10 split, filtered evaluation:
...
\bottomrule
\end{tabular}
\caption{Link-prediction results on the held-out test triples.}
\end{table}

The contrast is textbook (LO1): TransE's near-zero Hits@1 against ComplEx's
0.120 reflects the known weakness of translational models on 1-to-N relations --
a team has many players, a position many holders, so $h + r \approx t$ cannot
hold for all tails at once, while ComplEx's bilinear scoring can.
...
recovering them simulates an incomplete graph. Hits@10 $= 0.72$ (TransE) means a
missing link is recovered among the top 10 candidates 72\% of the time -- usable
...
Three honest limitations (LO6): (i) \emph{volume} -- 9.3k triples is small;
...
exactly the trade-off in Section~\ref{sec:datamodel}.
...
learned space is evaluated against the symbolic style clusters. That evaluation
is the most interesting single result: for 9 of 20 teams (ComplEx), the nearest
neighbour in embedding space lies in the same style cluster that the hand-built,
rule-derived metrics produce (random baseline about 4/20) -- the sub-symbolic
representation partially rediscovers the symbolic structure from passing,
position and pattern-participation topology alone. The comparison also shows the
division of strengths: logic gave exact, explainable, provably equivalent
results (Cypher $\equiv$ Datalog to the instance); embeddings gave generalisation
(link prediction for unseen edges) at the cost of explainability; and the graph
database contributed the systems dimension (indexes, query plans) that made both
feasible (LO12). Rule learning -- inducing the pattern definitions from data
instead of writing them -- would close the remaining loop and is the natural
extension.
